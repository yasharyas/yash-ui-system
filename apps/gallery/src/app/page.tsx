...
function FAQAccordionPreview() {
  return (
    <div style={{ transform: "scale(0.72)", transformOrigin: "top center", pointerEvents: "none" }}>
      <FAQAccordion
        title="Got Questions?"
        items={[
          { q: "Do you offer same-day delivery?", a: "Yes, for orders placed before 11 AM." },
          { q: "Can I customize my order?", a: "Absolutely! Contact us via WhatsApp." },
        ]}
      />
    </div>
  );
...
const popupPreviews: Record<string, () => React.ReactNode> = {
  ...
  "testimonial-card": () => (
    <div style={{ maxWidth: 320 }}>
      <TestimonialCard
        testimonial={{ rating: 5, text: "Absolutely loved it! The cake was divine. Will definitely order again.", name: "Priya Sharma", role: "Verified Buyer" }}
      />
    </div>
  ),
  "faq-accordion": () => (
    <div style={{ background: "white", width: "100%" }}>
      <FAQAccordion
        ...
        items={[
          { q: "Do you offer same-day delivery?", a: "Yes, for orders placed before 11 AM we offer same-day delivery within the city." },
          { q: "Can I customize my order?", a: "Absolutely! Contact us via WhatsApp and we'll make it perfect for you." },
...
const previews: Record<string, React.ReactNode> = {
  ...
  "testimonial-card": (
    <TestimonialCard
      testimonial={{ rating: 5, text: "The cake was absolutely divine!", name: "Priya Sharma", role: "Verified Buyer" }}
    />
  ),
  ...
  "whatsapp-fab": (
    <div className="flex items-center gap-2">
      <div className="w-12 h-12 bg-green-500 rounded-full flex items-center justify-center shadow-lg">
        <svg viewBox="0 0 24 24" className="w-6 h-6 fill-white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.127.558 4.122 1.532 5.859L.054 23.4a.5.5 0 0 0 .613.613l5.608-1.467A11.95 11.95 0 0 0 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22a9.95 9.95 0 0 1-5.095-1.395l-.364-.216-3.77.988.998-3.686-.236-.38A9.951 9.951 0 0 1 2 12C2 6.477 6.477 2 12 2s10 4.477 10 10-4.477 10-10 10z"/></svg>
      </div>
      <span className="text-xs text-neutral-500">WhatsApp FAB</span>
    </div>
  ),
  "image-placeholder": (
    <div className="flex gap-2">
      <ImagePlaceholder aspectRatio="1/1" className="w-16" rounded="rounded-lg" />
      <ImagePlaceholder aspectRatio="4/3" className="w-24" label="Photo" rounded="rounded-xl" />
    </div>
  ),
  "sticky-navbar": <StickyNavbarPreview />,
  "error-boundary": (
    <div style={{ transform: "scale(0.5)", transformOrigin: "top center", height: 120, pointerEvents: "none" }}>
